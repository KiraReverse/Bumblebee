...
class TkinterBot(customtkinter.CTk):
        ...
        while True:
            ...
            if x == None or y == None:
                ...
                if self.character.ac.goingtoportal or self.character.ac.gotoportal1 or self.character.ac.gotoportal2 or self.character.ac.gotoportal3 or self.character.ac.gotoportal4:
                    ...
                    if cctimer>1800: # 60sec * 30min = 1800sec
                        # cc=True
                        keyupall()
                        keyupall_arrow()
        ...
        while True:
            while self.pause:
                ...
                if self.stop_event.is_set():
                    return                    
            self.g.generate_newest_screenshot()
            die, red, gui, lie, whi = self.g.run_once_detect_all() #add gui
            ...
            if self.maplehwnd:
                ...
                if seperate_newldlocations:
                    ...
                    while True:
                        ...
                        try:
                            third_part = all_ld.split("|")[2]
                        except IndexError:
                            print("Split Captcha failed , try again.")
                            seperate_newldlocations = self.seperate_newld()
                            continue
                        print(f'Got Captcha : {third_part}')
                        position = win32gui.GetWindowRect(self.maplehwnd)
                        x, y, w, h = position
                        print(f'{x} {y} {w} {h}')   
                        await self.helper.move_to_and_click_and_move_away(x+417,y+380) #click type text captcha
                        ...
                        time.sleep(1.)
                        await self.helper.move_to_and_click_and_move_away(x+634,y+417) #click OK
                        ...
                        if not any(zip(*locations[::-1])):
                            ...
                            print("Captcha failed , pass again.")
                            seperate_newldlocations = self.seperate_newld()                           
                else:
                    print(f'no NEW LD')
            else:
    ...
    def seperate_newld(self):
        if self.maplehwnd == None:
            print(f'seperate_newld: Maplestory window not found.')
            return
        try:
            position = win32gui.GetWindowRect(self.maplehwnd)
            x, y, w, h = position
